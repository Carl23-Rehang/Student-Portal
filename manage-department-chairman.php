  ...
  if(trim($log_userid)!="") {
    if($_POST['btnsave']) {
      ...
      if($errn <= 0) {
        //
        // UPDATE
        $query = "UPDATE srgb.department SET deptchairman='" . $empid . "' WHERE LOWER(TRIM(deptcode))='" . strtolower(trim($id)) . "' ";
        $result = pg_query($pgconn,$query);
        //
        if ($result) {
          $dr = '
            <div class="alert alert-success alert-dismissible fade show" style="font-size: 0.75rem; padding: 0.6rem;" role="alert">
              <strong></strong> Department chairman updated.
...
<head>
  ...
  <title>Manage Department Chairman</title>
...
<body id="page-top">
                          ...
                          <div class="col-sm-6" style="padding-left: 4px;padding-right: 4px;">
                            ...
                            <div class="card shadow mb-4">
                              <div class="card-header py-3">
                                <div align="left">
                                  <h6 class="m-0 font-weight-bold text-primary" style="font-size: 0.7rem;"><span class="color-blue-1">Manage Department Chairman</span></h6>
                                </div>
                                  ...
                                  <div class="table-responsive">
                                    <table class="table">
                                      ...
                                      <tbody class="font-size-o-1">
                                        <?php
                                          $query = "SELECT * FROM srgb.department ORDER BY deptname ";
                                          $result = pg_query($pgconn, $query);
                                          if ($result) {
                                            $n = 0;
                                            while ($row = pg_fetch_array($result)) {
                                              $n++;
                                              //
                                              $code = trim($row['deptcode']);
                                              $desc = trim($row['deptname']);
                                              $college = trim($row['deptcoll']);
                                              $chairid = trim($row['deptchairman']);
                                              $chairname = "";
                                              //
                                              $squery = "SELECT * FROM pis.employee WHERE TRIM(UPPER(empid))=TRIM(UPPER('" . $chairid . "')) ";
                                              $sresult = pg_query($pgconn, $squery);
                                              if ($sresult) {
                                                while ($srow = pg_fetch_array($sresult)) {
                                                  $chairname = trim($srow['fullname']);
                                                }
                                              }
                                              //
                                              echo '
                                                  <tr style="">
                                                    ...
                                                    <td style="">' . $desc . '</td>
                                                    <td style="">' . $college . '</td>
                                                    <td style="">' . $chairname . '</td>
                                                    <td style="text-align: right;"><button type="button" class="btn btn-success btn-table-1" style="border-radius: 0px;" onclick="' . "loadDeptChairToField('" . $code . "','" . $chairid . "','" . $chairname . "','id','empid','employee');" . '" data-toggle="modal" data-target="#modalEdit">Update</button></td>
                                                  </tr>
                                              ';
                                              //
                                            }
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                      <div class="modal-content">
                                        <div class="modal-header bg-3 color-white-1 modal-header-1" style="padding-top: 12px;">
                                          <h5 class="modal-title" id="" style="font-size: 0.8rem;">Update Chairman</h5>
  ...
  <script>
    function loadDeptChairToField(id,empid,empname,targetID,targetEmpID,targetEmpName){
      try{
