<?php session_start(); include "connect.php"; //error_reporting(0);
  ...
  //
  //
  if((strtolower(trim($log_user_type)) == strtolower(trim("student")) || (strtolower(trim($log_user_type)) == strtolower(trim("employee")) && $log_user_role_isadmin <= 0)) && $setting_clearance_allowed <= 0) {
  ...
  //
  //
  if( trim($g_clearanceid) == "" ) {
    echo '<meta http-equiv="refresh" content="0;URL=clearance.php" />';
    exit();
  }
  //
  // GET SY SEM
  $fquery = "SELECT * FROM tbl_clearance WHERE active='1' and TRIM(LOWER(id))='" . trim(strtolower($g_clearanceid)) . "' ";
  $fresult = mysqli_query($conn, $fquery);
  if ($fresult) {
    while ($frow = mysqli_fetch_array($fresult)) {
      $g_clearance_sy = trim($frow['sy']);
      $g_clearance_sem = trim($frow['sem']);
    }
  }
  // CHECK SY, SEM
  if( ( trim($g_clearance_sy) != "" && trim($g_clearance_sem) != "" ) ) {
    if( strtolower(trim($log_user_type)) == strtolower(trim("student")) && ( $setting_clearance_restrict_sysem_student > 0 ) ) {
      if( trim(strtoupper($g_clearance_sy)) != trim(strtoupper($log_user_active_student_sy)) || trim(strtoupper($g_clearance_sem)) != trim(strtoupper($log_user_active_student_sem)) ) {
        echo '<meta http-equiv="refresh" content="0;URL=clearance.php" />';
        exit();
      }
    }
    if( strtolower(trim($log_user_type)) == strtolower(trim("employee")) && ( $setting_clearance_restrict_sysem_employee > 0 ) ) {
      if( trim(strtoupper($g_clearance_sy)) != trim(strtoupper($log_user_active_sy)) || trim(strtoupper($g_clearance_sem)) != trim(strtoupper($log_user_active_sem)) ) {
        echo '<meta http-equiv="refresh" content="0;URL=clearance.php" />';
        //echo $g_clearance_sy . " = " . $log_user_active_sy . " <> " . $g_clearance_sem . " = " . $log_user_active_sem;
        exit();
      }
    }
  }
            ...
            <div class="row">
                      <div class="col-sm-12" style="padding-left: 4px;padding-right: 4px;">
                        ...
                        <div align="left">
                                  ...
                                  <a class="btn btn-success btn-1 btn-width-min-1" style="font-size: 0.6rem; border-radius: 0px; border-top-right-radius: 16px; border-bottom-right-radius: 16px; margin-left: 12px; margin-bottom: 12px;" href="./clearance.php"><i class="fas fa-angle-double-left fa-sm"></i> <b>BACK TO CLEARANCE</b></a>
                          ...
                          <?php
                            ...
                            if (trim(strtolower($log_user_type)) == trim(strtolower("student"))) {
                              ...
                              $query = "SELECT 
                                          ...
                                          WHERE a.active='1' AND TRIM(UPPER(a.clearanceid))='" . trim(strtoupper($g_clearanceid)) . "' AND TRIM(UPPER(a.studid))='" . trim(strtoupper($log_userid)) . "' 
                              ...
                              if($n > 0) {
                                ...
                                echo '
                                      <div class="col-sm-12" style="margin-top: 10px; margin-bottom: 10px;">
                                        ...
                                        <div class="btn-c-1 btn-width-min-1" style="font-size: 0.6rem; color: #fff; background-color: #163763; border-radius: 0px; height: 28px; width: 120px; "> <b>On-going Tasks</b></div>
                                      </div>
                                ';
                                // END TILE
                                // SHOW RESULT
                              ...
                              if($n2 > 0) {
                                ...
                                echo '
                                      <div class="col-sm-12" style="margin-top: 20px; margin-bottom: 10px;">
                                        <div style=" width: 0; height: 0; border-bottom: 28px solid #163763; border-right: 18px solid transparent; position: absolute; margin-left: 120px;"></div>
                                        <div class="btn-c-1 btn-width-min-1" style="font-size: 0.6rem; color: #fff; background-color: #163763; border-radius: 0px; height: 28px; width: 120px; "> <b>Accomplished</b></div>
                                      </div>
                                ';
                                // END TILE
                                // ECHO NOT APPROVED
                            ...
                            if (trim(strtolower($log_user_type)) == trim(strtolower("employee"))) {
                              ...
                              $mquery = "SELECT 
                                            studid 
                                          FROM tbl_clearance_tasks 
                                          WHERE active='1' AND TRIM(UPPER(clearanceid))='" . trim(strtoupper($g_clearanceid)) . "' AND TRIM(UPPER(empid))='" . trim(strtoupper($log_userid)) . "' AND approv!='1' 
                                          GROUP BY studid ORDER BY entrydate ASC LIMIT 10 ";
                              $mresult = mysqli_query($conn, $mquery);
                              if ($mresult) {
                                while ($mrow = mysqli_fetch_array($mresult)) {
                                  //
                                  $mstudid = trim($mrow['studid']);
                                  //
                                  if(trim($mstudid) != "") {
                                    //
                                    $tas = 1;
                                    ...
                                    //
                                    $query = "SELECT 
                                                a.taskid,c.profilename,c.details AS profdesc,a.clearanceid,a.tasklistid,d.taskname,d.details AS taskdesc,a.empid,a.studid,a.notes,a.approv,a.approveddate,a.lockifprevnotapproved,a.locknextifnotapproved,a.unlockble,a.norder  
                                                FROM tbl_clearance_tasks AS a 
                                                LEFT JOIN tbl_clearance AS b ON b.id=a.clearanceid 
                                                LEFT JOIN tbl_clearance_profile AS c ON c.profileid=b.clearanceid 
                                                LEFT JOIN tbl_clearance_tasklist AS d ON d.tasklistid=a.tasklistid 
                                                WHERE a.active='1' AND TRIM(UPPER(a.clearanceid))='" . trim(strtoupper($g_clearanceid)) . "'  AND TRIM(UPPER(a.studid))='" . trim(strtoupper($mstudid)) . "' AND TRIM(UPPER(a.empid))='" . trim(strtoupper($log_userid)) . "' 
                                                ORDER BY a.norder ASC, d.taskname ASC ";
                                    $result = mysqli_query($conn, $query);
                                    if ($result) {
                                      while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        $tlnina = trim($row['locknextifnotapproved']);
                                        //
                                        $tlink = "./student-clearance.php?cid=" . $g_clearanceid . "&studid=" . $studid;
                                        //
                                        $thasprevtask = 0;
                                        $tprev_lnina = "";
                                        $tprev_lnina_approv = "";
                                        ...
                                        //
                                        $fresult = pg_query($pgconn, "SELECT studfullname from srgb.student where UPPER(TRIM(studid))='" . strtoupper(trim($studid)) . "' group by studfullname order by studfullname ASC ");
                                        //echo $log_u serid;
                                        if ($fresult) {
                                          while ($frow = pg_fetch_array($fresult)) {
                                            $studname = trim($frow['studfullname']);
                                          }
                                        }
                                        //
                                        //
                                        ...
                                        //echo "XXX";
                                        //echo " ORDER " . $tnorder;
                                        $squery = "SELECT * FROM tbl_clearance_tasks WHERE  active='1' AND TRIM(UPPER(clearanceid))='" . trim(strtoupper($g_clearanceid)) . "' AND TRIM(UPPER(studid))='" . trim(strtoupper($studid)) . "' AND TRIM(taskid)!='" . trim($tid) . "' AND norder<" . trim($tnorder) . " ORDER BY norder DESC LIMIT 1";
                                        $sresult = mysqli_query($conn, $squery);
                                        if ($sresult) {
                                          while ($srow = mysqli_fetch_array($sresult)) {
                                            $thasprevtask++;
                                            $tprev_lnina = trim($srow['locknextifnotapproved']);
                                            $tprev_lnina_approv = trim($srow['approv']);
                                            $tprevtask_empid = trim($srow['empid']);
                                            //echo $srow['taskid'] . " -- " . $tid . " -- " . $tprev_lnina;
                                            //echo $tprev_lnina_approv;
                                          }
                                        }
                                        //GET prev lnina
                                        $squery = "SELECT * FROM tbl_clearance_tasks WHERE  active='1' AND TRIM(UPPER(clearanceid))='" . trim(strtoupper($g_clearanceid)) . "' AND TRIM(UPPER(studid))='" . trim(strtoupper($studid)) . "' AND TRIM(taskid)!='" . trim($tid) . "' AND norder>" . trim($tnorder) . " ORDER BY norder ASC LIMIT 1";
                                        $sresult = mysqli_query($conn, $squery);
                                        if ($sresult) {
                                          while ($srow = mysqli_fetch_array($sresult)) {
                                            $thasnexttask++;
                                            $tnexttask_empid = trim($srow['empid']);
                                          }
                                        }
                                        //
                                        //
                                        ...
                                        if($ttaskid != "" && $ttaskname != "") {
                                          ...
                                          if($thasprevtask <= 0) {
                                            ...
                                            if(trim(strtoupper($tprevtask_empid)) != trim(strtoupper($empid))) {
                                              //echo " X1 ";
                                              if($tprev_lnina == "1") {
                                                //echo " X2 ";
                                                if($tprev_lnina_approv != "1") {
                                                  //echo " X3 ";
                                                  $tshow = 0;
                                                }
                                              }else{
                                          ...
                                          if($tshow == 1) {
                                            ...
                                            $tform = '
                                                      <!-- Modal -->
                                                      <div class="modal fade" id="modalUpdate_' . trim($n) . '" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                                          <div class="modal-content">
                                                            <div class="modal-header bg-3 color-white-1 modal-header-1" style="padding-top: 12px;">
                                                              <h5 class="modal-title" id="" style="font-size: 0.8rem;">Update Status</h5>
                                                              <button type="button" class="close color-white-1" style="padding-top: 4px; padding-bottom: 8px; margin-top: -12px;" data-dismiss="modal" aria-label="Close">
                                                                <span aria-hidden="true" style="font-size: 1.1rem;">&times;</span>
                                                              </button>
                                                            </div>
                                                            ...
                                                            <div class="modal-body" style="font-size: 0.7rem;">
                                                              <div align="left">
                                                                ...
                                                                <input type="hidden" name="taskid" value="' . trim($tid) . '" hidden />
                                                                ...
                                                                <div class="form-group">
                                                                  <span class="label1">Notes: <span class="text-danger"></span></span>
                                                                  <textarea class="form-control form-control-user  input-text-value font-size-o-1" name="notes" id="notes" placeholder="Notes">' . trim($tnotes) . '</textarea>
                                                                </div>
                                                            ...
                                                            <div class="modal-footer">
                                                              ...
                                                              <input type="submit" class="btn btn-success bg-2 font-size-o-1" style="border-radius: 0px; font-size: 0.6rem;" name="btnapprove" value="Update Clearance" />
                                                            </div>
                                                              </form>
                                                          </div>
                                                        </div>
                                            ...
                                            //
                                            $tbtn = '
                                                            <a href="#" class="btn btn-success bg-2 btn-width-min-1" style="font-size: 0.5rem; border-radius: 0px; border-top-right-radius: 0px; border-bottom-right-radius: 0px; margin-top: 8px; margin-bottom: 0px; margin-left: 0px; padding-top: 4px; padding-bottom: 4px;" data-toggle="modal" data-target="#modalUpdate_' . trim($n) . '"><i class="fas fa-edit fa-sm"></i> <b>UPDATE STATUS</b></a>
                                            ';
                                            if($tdisable == 1) {
                                              //
                                              $tlink = "#";
                                              ...
                                              $tcardclass1 = " border-left-secondary ";
                                              //
                                              $ttooltip = ' data-toggle="tooltip" title="This task is locked."  data-placement="bottom" ';
                                              //
                                              $tcursor = " cursor: not-allowed; ";
                                              //
                                            }
                                            ...
                                            echo '
                                              ...
                                              <div class="col-xl-2 col-md-6 mb-4">
                                                <div class="card ' . $tcardclass1 . ' shadow h-100 py-2">
                                                  <a class="link-card-1" href="' . $tlink . '" style="' . $tcursor . ' color: #bababf;"  ' . $ttooltip . ' >
                                                    <div class="card-body" style="padding-top: 8px; padding-bottom: 8px; min-height: 78px;">
                                                      <div class="row no-gutters align-items-center" style="vertical-align: top;">
                                                        <div class="col mr-2">
                                                          <div class="text-xs font-weight-bold ' . $tclass1 . ' text-uppercase mb-1" style="' . $tstyle1 . '">' . trim($studname) . '</div>
                                                          <div class="h5 mb-0 font-weight-bold ' . $tclass2 . '" style="font-weight: normal; font-style: italic; font-size: 0.7rem;' . $tstyle1 . '">' . trim($ttaskname) . '</div>
                                                          <div class="h5 mb-0" style="font-weight: nssormal; font-style: italic; font-size: 0.6rem; color: #bababf;">' . trim($tprofname) . '</div>
                                                          <div align="left">
                                                            ' . $tbtn . '
                                                          </div>
                                                        </div>
                                            ...
                                            if ($thasnexttask <= 0) {
                                              ...
                                              //echo "1";
                                              //echo $tnexttask_empid . " > " . $log_userid;
                                              if(trim(strtoupper($tnexttask_empid)) != trim(strtoupper($log_userid))) {
                                              //echo "2";
                                                break;
                                              }
                                            }
